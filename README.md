# AM1__AnandManikutty__profile__public

This GitHub repo has information for my LinkedIn.

[+]

This GitHub repo is a strict subset of the files in my other GitHub repo created to showcase information for LinkedIn:
 https://github.com/delphicventurescode/AM1__AnandManikutty__profile__public
 
[+]

Please review contents of the README.md file created for the other repository below (README.md as of November 18 2025).

This GitHib repo has a subset of the information described here in this README.md file.

[+]

# Anand Manikutty — Professional & Research Portfolio

This repository consolidates my professional materials, academic work, theoretical projects, and industry-facing profiles. It is designed as a single point of reference for recruiters, collaborators, research partners, and anyone interested in my work across technology, AI/ML, strategy, and the humanities.

The structure reflects the multi-disciplinary nature of my career:  
software engineering, AI/ML strategy, organizational leadership, academia, and original theoretical research.

---

## 📂 Repository Structure

### **1. `/it-professional-profile/`**
Industry-facing materials focused with my work spanning:
- Software engineering
- AI/ML strategy
- Product & platform leadership
- Information security
- CTO & Fractional CTO engagements

---

### **2. `/researcher-portfolio/`**
A collection of informational pointers, documents, summaries, and outlines related to my research and theoretical work:
- Political theory  
- Philosophy (La Gxoja Filozofio)
- Theory of the Firm
- Population of the Americas  
- Additional theoretical frameworks

Each project is presented in standalone `.md` files for clarity.

---

### **3. `/academicCV/`**

Contains my long-format academic CV, including:
- Research publications  
- Teaching experience    
- Talks, lectures, and theoretical contributions

---

### **4. `/papers/`**

Pointers to papers, as well as other material.

"Other material" include such materials as slide decks, conceptual summaries, and working documents across:
- Political science  
- Philosophy  
- Economics  
- Applied AI frameworks  
- Comparative theory

Every paper is referenced with complete citation details.
---

### **5. `/patents/`**
Information about patents I have authored or co-invented, including:
- U.S. Patent filings  

Areas of contribution include: databases, XML technologies, distributed systems.

---

## 📄 Root-Level Markdown Files

### **`README.md`**
The file you are currently reading — high-level overview of the repository.

### **`ZZ_PARA_linkedInAbout_3ParaVersion.md`**
A polished 3-paragraph version of my LinkedIn “About” section, optimized for recruiters and industry audiences.

### **`competency-summary.md`**
A competency-focused breakdown of my technical, strategic, and analytical strengths across engineering, product, AI/ML, and leadership domains.

### **`longFormatLinkedInHeadline.md`**
Long-format professional headline (industry optimized) designed for use in LinkedIn or professional summaries.

### **`longFormatLinkedInProfile.md`**
A detailed, narrative-style professional profile that integrates:
- Industry experience  
- Technical depth  
- Academic contributions  
- Cross-disciplinary research  

This is a more comprehensive version of my LinkedIn About section.

---

## 🎯 Purpose of This Repository

This repository serves as a **living portfolio** that brings together:
- Industry leadership experience  
- Technical depth in AI/ML, software engineering, and architecture  
- Academic and teaching contributions  
- Novel theoretical work (La Gravaj Teorioj Projektoj)  
- Writing, publications, and patents  

It is intended to provide a complete, transparent view of my professional and intellectual journey.

---

## 📬 Contact

For collaborations, consulting inquiries, research discussions, or invitations to speak:  
**LinkedIn:** https://www.linkedin.com/in/anandmanikutty/  
**Professional Profile:** https://bit.ly/3Hsylqd  

---

